...
function ModulesPage() {
  ...
  return (
    <Page
      kicker="目录"
      title="每一块积木都有抽屉"
      lead="仓库约 227 个包。这里收录主干与代表性插件。点组名进入该组的关系说明。"
    >
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row">
        <label className="block min-w-0 flex-1">
          <span className="sr-only">搜索包</span>
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="搜索包名、ctx、小孩版…"
        ...
        <p className="self-center text-sm text-muted tabular-nums">{filtered.length} 个</p>
      </div>

      <div className="mb-6 flex flex-wrap gap-2">
        <FilterChip label="全部层" on={layer === "all"} onClick={() => setLayer("all")} />
        {LAYERS.map((l) => (
          <FilterChip
            key={l.id}
      ...
      <h2 className="mt-12 mb-4 text-2xl">包卡片</h2>
      {filtered.length === 0 ? (
        ...
        {filtered.map((p) => (
          ...
          >
            ...
            <div className="text-sm text-muted">
              <p>{p.job}</p>
              {p.ctx ? <code className="mt-1 block font-mono text-[11px] text-subtle">{p.ctx}</code> : null}
            </div>
          </li>
        ))}
      </ul>
