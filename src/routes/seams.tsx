...
function SeamsPage() {
  ...
  return (
    <Page
      kicker="Seam"
      title="插座、插头、台灯"
      lead="一个 seam 必须有三种角色：定义接口、实现接口、使用接口。换提供方，消费方不用改。"
    >
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {SEAM_RULES.map((r) => (
          <article key={r.title} className="rounded-xl bg-surface p-5 shadow-[var(--shadow-border)]">
      ...
      <h2 className="mt-14 mb-4 text-2xl">点一个接头</h2>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <RoleCard title="定义" kid="插座长什么样" items={[{ pkg: seam.definition, note: seam.kid }]} />
        <RoleCard
          title="提供方"
          kid="插在哪栋楼"
      ...
      <h2 className="mt-14 mb-4 text-2xl">搬家演示：家 ↔ 云上小屋</h2>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <StayCard title="不动的" items={["agent-loop", "session 日志", "llm 调用", "skills"]} />
        <StayCard
          title={world === "home" ? "家里的执行世界" : "云上的执行世界"}
          items={
            world === "home"
              ? ["fs-local / fs-sandbox", "subprocess-local", "sandbox-local 围栏"]
              : ["fs-e2b", "subprocess-e2b", "ctx.e2b 生命周期"]
          }
        />
        <StayCard
          title="跟着走、不用改的工具"
          items={["tool-bash / bash-sandbox", "terminal-bash（PTY）", "lsp-stdio", "tool-fs"]}
        />
      </div>
      <TechNote className="mt-6">
        官方原话：existing dsh-bash-local、dsh-terminal-bash、dsh-lsp-stdio 不需要 E2B
        专用 fork。它们把执行世界操作委托给 ctx.fs 和 ctx.subprocess。ctx.sandbox 仍然只是本机
        confine(argv)，不是远程世界。
      </TechNote>
    </Page>
  );
