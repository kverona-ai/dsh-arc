...
function EventsPage() {
  return (
    <Page
      kicker="事件"
      title="三本喊话册"
      lead="选对事件域，是大多数改动的第一个决定。日记、对讲机、规矩，不要写错本子。"
    >
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {EVENT_DOMAINS.map((d) => (
          <article key={d.id} className="rounded-2xl bg-surface p-5 shadow-[var(--shadow-border)] sm:p-6">
            ...
            <p className="mt-1 text-sm text-muted">何时：{d.when}</p>
      ...
      <h2 className="mt-14 mb-4 text-2xl">工具流水线</h2>
      ...
      <h2 className="mt-14 mb-4 text-2xl">四种喊法再看一眼</h2>
      ...
      <TechNote className="mt-8">
        SDK 若要可回放 transcript，应消费 session/event。agent/*
        是队列、状态、拦截、请求构造、steering 和错误处理的实时接口。完整生产者/消费者矩阵见
